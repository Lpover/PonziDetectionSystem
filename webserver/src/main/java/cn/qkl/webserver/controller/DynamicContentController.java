...
public class DynamicContentController {
    ...
    @ApiOperation("动态风险数字内容批量纠错/人工修正")
    @PutMapping("content/type/manual")
    public BaseResult<Void> manualReviseDynamicType(@Validated ManualReviseDynamicTypeDTO dto) {
        return BaseResult.ok();
    }

    @ApiOperation("动态风险数字内容重新识别")
    @PutMapping("content/type/reCheck")
    public BaseResult<Void> reCheckDynamicType(@Validated ReCheckDynamicTypeDTO dto) {
        return BaseResult.ok();
    }

    @ApiOperation("动态风险数字内容批量导出")
    @PostMapping("content/export")
    public void userListExport(@RequestBody @Validated DynamicContentExportDTO dto, HttpServletResponse response) {
